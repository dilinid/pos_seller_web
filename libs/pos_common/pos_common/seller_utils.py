"""Small helpers shared by the seller fulfillment services (picking, packing, ordering)."""
...
def short_user_code(user_id: Optional[int]) -> Optional[str]:
    """Fits the varchar(10) *_user/*_mdby columns used across pos_itempick/pos_itempack."""
    return str(user_id)[:10] if user_id is not None else None
