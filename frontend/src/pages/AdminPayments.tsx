...
const PAYOUT_TABS: Array<{ key: PayoutStatus | 'all'; label: string }> = [
  { key: 'all', label: 'All' },
  { key: 'paid', label: 'Paid' },
...
function getOrderPayoutStatus(order: Order): PayoutStatus {
...
const AdminPayments: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    for (const order of orders) {
      const amount = order.items.reduce((sum, i) => sum + i.price * i.quantity + i.deliveryFee, 0);
      const status = getOrderPayoutStatus(order);
      s.total++;
  ...
  const filteredOrders = useMemo(() => {
    let result = orders;
    if (activeTab !== 'all') {
      result = result.filter((o) => getOrderPayoutStatus(o) === activeTab);
    }
    if (searchQuery.trim()) {
