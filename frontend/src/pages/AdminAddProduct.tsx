...
import { SuccessOverlay } from '../components/admin/SuccessOverlay';
import type { StoreProfile } from '../types/store.type';

function hasDeliveryConfig(profile: StoreProfile): boolean {
  return Object.keys(profile.districtFees ?? {}).length > 0
    && (profile.weightFeeBrackets ?? []).length > 0;
}

function hasPickupConfig(profile: StoreProfile): boolean {
...
const AdminAddProduct: React.FC = () => {
  ...
  const storeProfile = useStoreStore((s) => s.profile);
  ...
  const validate = (): string | null => {
    ...
    if (deliveryAvailable && !hasDeliveryConfig(storeProfile)) {
      return 'Configure your delivery settings (district fees and weight brackets) in your Store Profile before offering delivery.';
    }
    if (pickupAvailable && !hasPickupConfig(storeProfile)) {
      return 'Add your pickup address in your Store Profile before offering pickup.';
    }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          <button onClick={() => navigate('/admin/products')} className="prod-back-btn">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '800px', margin: '0 auto' }}>
        ...
        <div className="prod-section-card">
          ...
          {deliveryAvailable && !hasDeliveryConfig(storeProfile) && (
            <div style={{
              ...
              <Link to="/admin/settings" style={{ color: 'var(--primary)', fontWeight: 600 }}>
                Store Profile
              </Link>{' '}
              before offering delivery.
            </div>
          )}
          {pickupAvailable && !hasPickupConfig(storeProfile) && (
            <div style={{
              padding: '10px 14px', borderRadius: '10px', background: '#fffbeb',
              border: '1px solid #fde68a', fontSize: '0.82rem', color: '#92400e',
            }}>
      ...
      <div className="prod-form-footer">
        <button onClick={() => navigate('/admin/products')} className="btn btn-secondary" style={{ padding: '10px 20px', borderRadius: '10px', fontSize: '0.85rem' }}>
          Cancel
        </button>
