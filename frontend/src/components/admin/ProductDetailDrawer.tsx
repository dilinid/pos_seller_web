...
export const ProductDetailDrawer: React.FC<ProductDetailDrawerProps> = ({
  ...
  const districtFeeEntries = Object.entries(storeProfile.districtFees ?? {});
  const pickupAddress = storeProfile.pickupAddress || '';
  const estimatedDeliveryDays = storeProfile.estimatedDeliveryDays || '';
  const freeDeliveryMin = storeProfile.freeDeliveryMin ?? null;
          ...
          {draft.status === 'draft' && (
            <>
              <div style={{ display: 'flex', gap: '8px', flex: 1 }}>
                <button onClick={() => navigate(`/admin/products/edit/${draft.id}`)} className="prod-action-btn">
